...
| One-command public verifier available | PASS | `python scripts\public_verify.py --out <temp>` writes `public_verify_report.json` outside the repository, refuses repo-internal output by default, and reports `workspace_state` so dirty branches are visible before release. |
