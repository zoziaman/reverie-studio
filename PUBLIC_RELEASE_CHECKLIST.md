...
| One-command public verifier available | PASS | `python scripts\public_verify.py --out <temp>` writes `public_verify_report.json` outside the repository, refuses repo-internal output by default, and reports `workspace_state` as counts/fingerprints so dirty branches are visible before release without copying raw local path names into the report. |
