...
def test_setup_env_launcher_runs_direct_script():
    launcher = _launcher_text("run_reverie_setup_env.bat")

    assert 'cd /d "%~dp0"' in launcher
    assert 'python "%~dp0src\\reverie_env_bootstrap.py" --json' in launcher
    assert "pause" in launcher
