...
def test_actor_episode_variant_coverage_report_tracks_generated_supplement_assets(tmp_path):
    actor_model = _actor_model_module()
    actor_root = tmp_path / "actor_models"
    plan = actor_model.build_pack_actor_roster_plan(
        "daily_life_toon",
    ...
    missing_report = actor_model.build_actor_episode_variant_coverage_report(
        request_manifest,
        actor_root=actor_root,
    )
    ...
    ready_report = actor_model.build_actor_episode_variant_coverage_report(
        request_manifest,
        actor_root=actor_root,
    )

    assert missing_report["schema"] == "reverie.pack.actor_episode.variant_coverage.v1"
    assert missing_report["expected_count"] == 1
    assert missing_report["missing_count"] == 1
    assert missing_report["ready_for_episode"] is False
    assert missing_report["missing_variants"] == ["actor_daily_adult_man_01:surprised_jumping"]
